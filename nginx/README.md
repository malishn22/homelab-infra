...
The `.home` suffix is not real DNS. Each client resolves them through its own
...
`calimali-api` vhost is rendered only at container start (see below), so CI's
`nginx -t` never sees it and needs no stub either.
...
**Public hostnames stay out of git.** `calimali-api` is defined in
`templates/*.template`, taking its `server_name` from `.env` (`CALIMALI_PUBLIC_HOST`). At
container start the nginx image's `envsubst` renders it into `conf.d/`
(its default output dir); the resulting `conf.d/calimali-api.conf` is **gitignored**, so
the real hostname is never committed. `NGINX_ENVSUBST_FILTER` restricts substitution to that
var, leaving nginx's own `$host`, `$scheme`, `$http_upgrade`, etc. intact.
